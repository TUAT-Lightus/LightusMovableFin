...
namespace RegAddr {
namespace Accel {
constexpr uint8_t ACCD_X_LSB = 0x02;
constexpr uint8_t PMU_RANGE  = 0x0f;
constexpr uint8_t ACCD_HBW   = 0x13;
}  // namespace Accel
namespace Gyro {
...
void BMX055::begin(TwoWire &wire, AccelRange accelRange, GyroRange gyroRange, MagneticRate magneticRate,
                   FilterConfig filter) {
    wire_         = &wire;
    accelRange_   = accelRange;
    gyroRange_    = gyroRange;
    magneticRate_ = magneticRate;
    WireHelper::setVal(*wire_, ACCL_ADDRESS_, RegAddr::Accel::PMU_RANGE, static_cast<uint8_t>(accelRange_));
...
BMX055::DataType BMX055::read() {
    BMX055::DataType result;
...
BMX055::RawDataType BMX055::read16() {
    BMX055::RawDataType result;
...
void BMX055::BMM150TrimData::toJSON(JsonObject obj) {
...
void BMX055::dumpAccelCompensationParametersToJson_(JsonObject obj) { obj["G_PER_LSB"] = G_PER_LSB_(); }
void BMX055::dumpGyroCompensationParametersToJson_(JsonObject obj) { obj["DPS_PER_LSB"] = DPS_PER_LSB_(); }
void BMX055::dumpMagCompensationParametersToJson_(JsonObject obj) { magTrimData_.toJSON(obj); }
String BMX055::readCompensationParametersJSON() {
...
void BMX055::dumpCompensationParametersToJSON(JsonObject obj) {
    obj["version"] = 0;
    dumpAccelCompensationParametersToJson_(obj["accel"].to<JsonObject>());
    dumpGyroCompensationParametersToJson_(obj["gyro"].to<JsonObject>());
    dumpMagCompensationParametersToJson_(obj["mag"].to<JsonObject>());
}
Vec3<int16_t> BMX055::readAccel16() { return readAccel16_(); }
Vec3<int16_t> BMX055::readGyro16() { return readGyro16_(); }
Vec3<int16_t> BMX055::readMag16() { return readMag16_(); }
Vec3<float> BMX055::convertAccelFrom16(Vec3<int16_t> accel16, AccelRange range) {
    return (accel16)*G_PER_LSB_(range) / 1000;
}
Vec3<float> BMX055::convertGyroFrom16(Vec3<int16_t> gyro16, GyroRange range) { return gyro16 * DPS_PER_LSB_(range); }
Vec3<int16_t> BMX055::readAccel16_() {
    int16_t ax16, ay16, az16;
    uint8_t buf[6];
    WireHelper::readDataRaw(*wire_, ACCL_ADDRESS_, RegAddr::Accel::ACCD_X_LSB, buf, 6);
...
Vec3<int16_t> BMX055::readGyro16_() {
    int16_t gx16, gy16, gz16;
    uint8_t buf[6];
    WireHelper::readDataRaw(*wire_, GYRO_ADDRESS_, RegAddr::Gyro::RATE_X_LSB, buf, 6);
...
Vec3<int16_t> BMX055::readMag16_() {
    int16_t mx16, my16, mz16;
    uint8_t buf[6];
    WireHelper::readDataRaw(*wire_, MAGN_ADDRESS_, 0x42, buf, 6);
...
Vec3<float> BMX055::CompensateMag_(Vec3<int16_t> mag, uint16_t rhall) {
    Vec3<float> result;
...
float BMX055::CompensateMagX_(int16_t raw_mag_x, uint16_t rhall) {
    float retval = 0;
...
float BMX055::CompensateMagY_(int16_t raw_mag_y, uint16_t rhall) {
    float retval = 0;
...
float BMX055::CompensateMagZ_(int16_t raw_mag_z, uint16_t rhall) {
    float retval = 0;
...
float BMX055::G_PER_LSB_() const { return G_PER_LSB_(accelRange_); }

float BMX055::G_PER_LSB_(AccelRange accelRange) {
...
float BMX055::LSB_PER_DPS_() const { return LSB_PER_DPS_(gyroRange_); }
float BMX055::LSB_PER_DPS_(GyroRange gyroRange) {
...
float BMX055::DPS_PER_LSB_() const { return DPS_PER_LSB_(gyroRange_); }
float BMX055::DPS_PER_LSB_(GyroRange gyroRange) { return 1 / LSB_PER_DPS_(gyroRange); }
float BMX055::uT_PER_LSB_() const { return 1 / 16.0; }

void BMX055::setAccelAddress(uint8_t addr) { ACCL_ADDRESS_ = addr; }
void BMX055::setGyroAddress(uint8_t addr) { GYRO_ADDRESS_ = addr; }
void BMX055::setMagnAddress(uint8_t addr) { MAGN_ADDRESS_ = addr; }
uint8_t BMX055::accelAddress() const noexcept { return ACCL_ADDRESS_; }
uint8_t BMX055::gyroAddress() const noexcept { return GYRO_ADDRESS_; }
uint8_t BMX055::magnAddress() const noexcept { return MAGN_ADDRESS_; }
int BMX055::absAccelMax() {
    switch (accelRange_) {
        case AccelRange::PM2G:
            return 2;
        case AccelRange::PM4G:
...
BMX055::AccelRange BMX055::accelRangeFromAbsAccelMax(int absAccelMax) {
    switch (absAccelMax) {
        case 2:
...
BMX055::GyroRange BMX055::gyroRangeFromAbsGyroMax(int absGyroMax) {
    switch (absGyroMax) {
        case 125:
...
Vec3<float> BMX055::readAccel() { return convertAccelFrom16(readAccel16_(), accelRange_); }
Vec3<float> BMX055::readGyro() { return convertGyroFrom16(readGyro16_(), gyroRange_); }
Vec3<float> BMX055::readMag() { return CompensateMag_(readMag16_(), readRhall16_()); }
}  // namespace Lightus::Sensors

#if !defined(LIGHTUS_SENSORS_NO_BMX055_INSTANCE) && !defined(LIGHTUS_SENSORS_NO_GLOBAL_INSTANCES)
Lightus::Sensors::BMX055 bmx055;
#endif
